...
| `/etc/infisical/pkcs11.conf` | Infisical library config | User-supplied (or via `INFISICAL_CONFIG` env var). **Required** for the Infisical provider — the library returns `CKR_GENERAL_ERROR` from `C_Initialize` if the config file is missing or unreadable. Minimum: `{"server_url": "https://app.infisical.com"}`. |
...
| Infisical config file missing | Library returns `CKR_GENERAL_ERROR` from `C_Initialize` (no log file created). User must supply a valid `INFISICAL_CONFIG` file. |
